# -*- coding: utf-8 -*-
"""Copy of mc-recover.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Bn2evI95j7Bvi8wn6O0sXiR-9L0Mn1kS
"""
...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_edge_nf, hidden_node_nf, hidden_coord_nf,edges_in_d=0,
        ...
        self.edge_mlp = nn.Sequential(
            nn.Linear(input_edge + 1 + (num_vectors_in)*(hidden_edge_nf//2), hidden_edge_nf),
            act_fn,
...
class E_GCL_mask(E_GCL):
    ...
    def __init__(self, input_nf, output_nf, hidden_edge_nf, hidden_node_nf, hidden_coord_nf,
                ...
                num_vectors_in=1, num_vectors_out=1, sin_embedding=False,
        ...
        self.act_fn = act_fn
        if sin_embedding:
            self.sin_embedding = SinusoidsEmbeddingNew(size=num_vectors_out)
            self.hidden_edge_nf = 2*self.sin_embedding.dim
        else:
            self.sin_embedding = None
            self.hidden_edge_nf = edges_in_d
    ...
    def forward(self, h, edge_index, coord, node_mask, edge_mask, edge_attr=None, node_attr=None, n_nodes=None, sin_embedding=None):
        row, col = edge_index
        radial, coord_diff = self.coord2radial(edge_index, coord)

        if sin_embedding is not None:
            radial = sin_embedding(radial)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_edge_nf, hidden_node_nf, hidden_coord_nf,
        ...
        if sin_embedding:
            self.sin_embedding = SinusoidsEmbeddingNew(size=1)
            self.hidden_edge_nf = 2*self.sin_embedding.dim
        else:
            self.sin_embedding = None
            self.hidden_edge_nf = in_edge_nf
        ### Encoder
        ...
        self.add_module("gcl_%d" % 0,
                E_GCL_mask(self.hidden_node_nf, self.hidden_node_nf,
                    ...
                    edges_in_d=self.hidden_edge_nf, nodes_attr_dim=n_node_attr,
                    act_fn=act_fn, recurrent=True,
                    coords_weight=coords_weight, attention=attention,
                    num_vectors_in=1, num_vectors_out=num_vectors, update_coords=update_coords,sin_embedding=sin_embedding ))
        for i in range(1, n_layers - 1):
            self.add_module("gcl_%d" % i,
                E_GCL_mask(self.hidden_node_nf, self.hidden_node_nf,
                    ...
                    act_fn=act_fn, recurrent=True,
                    coords_weight=coords_weight, attention=attention,
                    num_vectors_in=num_vectors, num_vectors_out=num_vectors, update_coords=update_coords,sin_embedding=sin_embedding ))
        self.add_module("gcl_%d" %  (n_layers - 1),
            E_GCL_mask(self.hidden_node_nf, self.hidden_node_nf,
                ...
                update_coords=update_coords, last_layer=True,sin_embedding=sin_embedding ))
    ...
    def forward(self, h0, x, edges, edge_attr, node_mask=None, edge_mask=None, n_nodes=5):
        h = self.embedding(h0)
        for i in range(0, self.n_layers):
            if self.node_attr:
                h, x, _ = self._modules["gcl_%d" % i](h, edges, x, node_mask, edge_mask, edge_attr=edge_attr, node_attr=h0, n_nodes=n_nodes, sin_embedding=self.sin_embedding)
            else:
                h, x, _ = self._modules["gcl_%d" % i](h, edges, x, node_mask, edge_mask, edge_attr=edge_attr,
                                                      node_attr=None, n_nodes=n_nodes, sin_embedding=self.sin_embedding)
...
class SinusoidsEmbeddingNew(nn.Module):
    def __init__(self, max_res=15., min_res=15. / 2000., div_factor=4, size=1):
        super().__init__()
        self.n_frequencies = int(math.log(max_res / min_res, div_factor)) + 1
        frequencies = 2 * math.pi * div_factor ** torch.arange(self.n_frequencies)/max_res
    ...
    def forward(self, x):
        x = torch.abs(x).unsqueeze(-1)  # Use abs instead of sqrt to handle negative values

        # Reshape frequencies to match input dimensions
        frequencies = self.frequencies.view(1, 1, -1).repeat(1, self.size, 1)

        emb = x * frequencies
        emb = torch.cat((emb.sin(), emb.cos()), dim=-1).squeeze()
